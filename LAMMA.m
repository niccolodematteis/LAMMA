...
%--------------------------------------------------------------------------
    function [regularGrid, Pool]=CheckInputCorrectness
        ...
        %======= FORSE HO TROVATO UNA FUNZIONE ALTERNATIVA
        %check whether the Statistics Toolbox is installed for using
        %knnsearch to find the nearest neighbours. Otherwise throw an error
        if ~license('test','Statistics_Toolbox')
            error('The Statistics and Machine Learning Toolbox is required to search for the nearest neighbours')
        end
...
%             %check the number of neighbours which must be 4
%             if neigh~=4
%                 warning('With regular grid the number of neighbours must be 4')
%             end
